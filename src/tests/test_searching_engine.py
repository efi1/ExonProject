...
def test_search_results(search_client, cfg_data):
    """
    ...
    search_client.update_ranking_job
    res = search_client.get_search_term_options('leisure time')
    assert res['status'] == 'success', F"Error occurred {res['data']}"
    assert res['data'] == cfg_data['result'], F"wrong results; expected: {cfg_data['result']}, actual: {res['data']}"
